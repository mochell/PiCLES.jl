...
"""
This verison uses static winds as forcing for the time of the ODE integrations.
    Winds (and all other forcing) is provided through a ODEForcing struct, which is passed to the ODE function. 
    this makes the ODE intergation purely local.
"""
...
export particle_equations, ODESettings, ForcingData
using LinearAlgebra
using StaticArrays
...
"""
ForcingData
Mutable structure to hold forcing data that changes each timestep
# Fields  
$(DocStringExtensions.FIELDS)
"""
@with_kw mutable struct ForcingData
    "Zonal wind component [m/s]"
    u_wind::Float64 = 0.0
    "Meridional wind component [m/s]"
    v_wind::Float64 = 0.0
    "Zonal current component [m/s]"
    u_current::Union{Float64,Nothing} = nothing
    "Meridional current component [m/s]"
    v_current::Union{Float64,Nothing} = nothing
    "Sea ice concentration [0-1]"
    sea_ice_concentration::Union{Float64,Nothing} = nothing
    "Sea ice thickness [m]"
...
function Base.show(io::IO, ow::ForcingData)
    ...
    # Always show wind components
    push!(lines, "├── u_wind: $(ow.u_wind) m/s")
    push!(lines, "├── v_wind: $(ow.v_wind) m/s")
    
    # Only show optional fields if they are not nothing
    if ow.u_current !== nothing
        push!(lines, "├── u_current: $(ow.u_current) m/s")
    end
    
    if ow.v_current !== nothing
        push!(lines, "├── v_current: $(ow.v_current) m/s")
    end
    
    if ow.sea_ice_concentration !== nothing
        push!(lines, "├── sea_ice_concentration: $(ow.sea_ice_concentration)")
    end
    
    if ow.sea_ice_thickness !== nothing
        push!(lines, "└── sea_ice_thickness: $(ow.sea_ice_thickness) m")
    else
        # Fix the last line connector if sea_ice_thickness is nothing
        if length(lines) > 1
...
"""
    validate_forcing(forcing::ForcingData)

Validates that required forcing fields are properly set.
Wind components (u_wind, v_wind) are mandatory.
"""
function validate_forcing(forcing::ForcingData)
    if isnan(forcing.u_wind) || isnan(forcing.v_wind)
        throw(ArgumentError("Wind forcing components u_wind and v_wind must be valid numbers, not NaN"))
    end
    
    # Optional: warn about very large wind speeds (might indicate unit issues)
    if abs(forcing.u_wind) > 100 || abs(forcing.v_wind) > 100
        @warn "Very large wind speeds detected: u_wind=$(forcing.u_wind), v_wind=$(forcing.v_wind). Check units (should be m/s)."
    end
    
    return true
end

@inline _zero_pc(_) = 0.0
...
"""
function particle_equations(; γ::Number=0.88, q::Number=-1 / 4.0, IDConstants=IDConstants(),
    propagation=true,
    input=true,
    dissipation=true,
    peak_shift=true,
    ...
    function particle_system(dz, z, params, forcing, t)#::MVector{5, Number}
        ...
        M     = haskey(params, :M) ? params.M : nothing # Projection matrix dependent on GridType
        ...
        PropagationCorrection = haskey(params, :PC) ? params.PC : _zero_pc # Great circle correction for specific grid types  <--------- test this!

        # Get forcing data either from functions or from passed data structure
        if use_static_forcing && forcing !== nothing
            # Validate forcing data on first call (when needed)
            validate_forcing(forcing)
            u = forcing.u_wind
            v = forcing.v_wind
        elseif use_static_forcing && forcing === nothing
            throw(ArgumentError("use_static_forcing=true but no forcing provided"))
        else
        ...
        αp = αₚ(u, v, c_gp_x, c_gp_y)
        Hₚ = H_β(αp, p)
        Δₚ = Δ_β(αp) ## <--- this one is slow!!
        ...
        # energy
        ωrgS = ωₚ * r_g * S_cg_tilde
        dz[1] = ωrgS + ωₚ * (Ĩ - D̃) #- c̄ .* G_n,

        # peak group velocity vector
        dz[2] = -c̄_x * ωrgS + c̄_y * S_dir_tilde + c̄_y * S_sphere_tilde
        dz[3] = -c̄_y * ωrgS - c̄_x * S_dir_tilde - c̄_x * S_sphere_tilde
        ...
        if propagation
            if M === nothing
                ...
                dz[4] = M[1, 1] * c̄_x + M[1, 2] * c̄_y
                dz[5] = M[2, 1] * c̄_x + M[2, 2] * c̄_y
            end
        else
            dz[4] = 0.0
            dz[5] = 0.0
...
#     ###  ---------------- start function here
#     function partice_system(dz, z, params, forcing, t) #<: Vector{Number}
#         #unpack0
#         lne, c̄_x, x      = z
...
function particle_rays(info=false)
    ...
    #D = Differential(t)
    function partice_system(dz, z, params, t, forcing=nothing) #<: Vector{Number}
        lne, c̄_x, x = z
        # energy
        dz[1] = 0
        # peak group velocity vector
