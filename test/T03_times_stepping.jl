...
u(x, y, t) = IfElse.ifelse.(x .< 180e3, U10, 0.00) + y * 0.0 + t * 0.0
v(x, y, t) = IfElse.ifelse.(x .< 180e3, V10, 0.00) + y * 0.0 + t * 0.0 .* cos(t * 1 / (5 * 60 * 60 * 2π))

# u(x, y, t) = U10 + x * 0.0 + y * 0.0 + t * 0.0
# v(x, y, t) = V10 + x * 0.0 + y * 0.0 + t * 0.0
winds = (u=u, v=v)

grid = TwoDCartesianGridMesh(400e3, 41, 200e3, 21, periodic_boundary=(false, false))
...
wave_model = WaveGrowthModels2D.WaveGrowth2D(; grid=grid,
    ...
    ODEinit_type="wind_sea",  # default_ODE_valuves
    #ParticleDefaults2D(default_windsea[1], default_windsea[2], default_windsea[3], 0.0, 0.0),
...
t_end = wave_simulation.model.clock.time + wave_simulation.Δt

position_xy = wave_simulation.model.ParticleCollection.position_xy

wave_simulation.model.winds.u(4, 100e3, t_end)

function get_wind_field_at_positions(u_comp, position_xy, t_end)
    return [u_comp(position_xy[i, j][1], position_xy[i, j][2], t_end) for i in 1:size(position_xy, 1), j in 1:size(position_xy, 2)]
end

speed_square(x::Float64, y::Float64) = x^2 + y^2


uu = get_wind_field_at_positions(wave_simulation.model.winds.u, position_xy, t_end)
vv = get_wind_field_at_positions(wave_simulation.model.winds.v, position_xy, t_end)


wind_min_squared = 0.5^2

enough_wind_at_tend = speed_square.(uu, vv) .>= wind_min_squared

heatmap(transpose(enough_wind_at_tend))

findall(enough_wind_at_tend)

@info wave_simulation.model.ocean_points

heatmap(transpose(wave_simulation.model.ParticleCollection.on))
